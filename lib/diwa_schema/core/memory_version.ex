defmodule DiwaSchema.Core.MemoryVersion do
  ...
  @derive {Jason.Encoder, only: [:id, :memory_id, :content, :tags, :metadata, :operation, :actor, :reason, :parent_version_id, :inserted_at]}

  schema "memory_versions" do
    field :content, :string
    field :tags, {:array, :string}, default: []
    field :metadata, :map, default: %{}
    field :operation, :string # "create", "update", "delete"
    field :actor, :string
    field :reason, :string
  ...
  def changeset(memory_version, attrs) do
    memory_version
    |> cast(attrs, [:memory_id, :content, :tags, :metadata, :operation, :actor, :reason, :parent_version_id])
